...
for (const [langLabel, langCode] of Object.entries(languageMap)) {
  test(`Joke is displayed in ${langLabel}`, async ({ page }) => {
    ...
    // await page.goto('http://127.0.0.1:8000/', { timeout: 15000, waitUntil: 'domcontentloaded' }); // local run

    // Select the language in dropdown
    const dropdown = page.locator('#language-selection'); // ✅ match actual ID
    await expect(dropdown).toBeVisible();
    await dropdown.selectOption(langLabel);

    // Click "Get Joke"
    const jokeButton = page.locator('#get-joke'); // ✅ replace with actual button ID
    await expect(jokeButton).toBeVisible();
    await jokeButton.click();

    // Wait for the joke display
    const jokeDisplay = page.locator('#joke-text'); // ✅ replace with actual container ID
    ...
    // Debug log
    console.log(`Selected: ${langLabel}, Expected: ${langCode}, Detected: ${detectedLang}, Joke: ${jokeText}`);
